...
Rust + Tauri 2 + Vue 3 + JavaScript。支持透明无边框桌宠、鼠标跟随、点击表情、气泡菜单、独立设置窗口、Rust 托盘、30/15 FPS 和隐藏时停更。使用 4K 运行贴图。
...
接着读 [第三步：模型接入](../docs/learning/03-model.md) 和 [第四步：窗口与托盘](../docs/learning/04-desktop.md)。左键单击角色随机切换表情，按住移动超过 6 个逻辑像素后拖动窗口。右键角色展开五个球：表情、设置、置顶、隐藏、退出。

“表情”在原来五个位置换成微笑、眯眼、泪眼、泪滴、返回；“设置”直接打开独立窗口，可调整帧率和置顶。Esc 逐层返回，右键或失焦收起。屏幕右边空间不足时菜单翻到左侧，上下空间不足时使用紧凑排列，可滚动查看全部入口。底部工具栏已移除，开发模式的“联调”和“重新加载模型”位于设置窗口。调用链和验收记录见 [气泡菜单](../docs/learning/07-bubble-menu.md)。

右键系统托盘图标打开原生菜单，左键显示桌宠；隐藏后仍可通过托盘找回。
