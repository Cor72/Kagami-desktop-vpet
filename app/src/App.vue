...
const pet = usePet()
const { expressionRequest, renderPolicy, ready, settingsBusy, error, setVisible, quit } = pet
...
<template>
  <main :class="{ 'panel-open': panelOpen }">
    <PetStage :expression-request="expressionRequest" :render-policy="renderPolicy" @error="error = $event" />
