...
**互斥怎么做**：`App.vue` 已经持有气泡菜单的开关状态（`petMenu.js` 的状态机在它手里），
把气泡的显示条件挂在同一处即可，不要在 `PetSpeechBubble.vue` 内部再引一份菜单状态。
菜单打开时清掉当前气泡并丢弃新来的气泡。

**窗口隐藏/最小化时要停**：`settings.rs` 里已有 `visible` 状态，
Rust 侧的采样循环挂在它上面——不可见时不采样、不判断、不发言。
...
| **窗口标题** | 两档：① **已知编辑器**（VS Code / JetBrains / Sublime / Vim）取第一段当文件名；② **浏览器**先用白名单认站点，**命中才**把页面标题当内容——白名单只匹配标题**最后两段**，所以页面标题里出现站点名不算数。其余应用（聊天软件等）**整条丢弃** |
| **浏览器标题开关** | 单独一个设置项（默认开）。关掉之后引擎**连看都不看**那一项，不是看了再忽略。页面标题会**发给第三方模型服务**，这条通道必须能一句话关掉 |
...
| 回到活跃 | 内置模板表 |
| **前台窗口切换到某个程序** | 「进程名 → 文案」映射表 |

番茄钟相关的触发**不做**（本计划不含番茄钟）。

**第三条要用「前台窗口变化」，不要用「轮询进程表检测新进程」。** 理由：

- 前者只需要 `GetForegroundWindow`，本来就是每次采样都在读的；后者要枚举全部进程再比对，贵且没必要
- 用户「打开某个程序」时，那个窗口几乎必然成为前台窗口，效果一样
- 更重要的是：**前台窗口变化 = 用户的注意力刚刚转移**，这才是适合搭话的时刻。
  一个在后台悄悄启动的进程（比如自动更新程序）不该触发说话

实现上就是：本次采样的前台进程名 ≠ 上次采样的前台进程名 → 触发。
...
**文案规则（重要，直接决定这个功能是"有生气"还是"烦人"）**：

| 规则 | 为什么 |
|---|---|
| **用陈述句，不要用问句** | 问句（「要不要休息一下？」）会制造回应的义务。陈述句（「看起来在忙代码」）说完就完，用户不理它也没有任何负担 |
| 一句话，不超过约 20 字 | 气泡显示不下，长了会挤占画面 |
| 不评价、不催促、不给建议 | 它不是教练。一旦开始说「你该休息了」，性质就变成说教了 |
| 同一个程序准备 3–5 条轮换 | 每次都说一模一样的话，两天就腻了 |

**内置映射表的起步清单**（按进程名匹配，不区分大小写）：

| 分类 | 进程名示例 | 说明 |
|---|---|---|
| 代码编辑器 | `Code.exe`、`idea64.exe`、`sublime_text.exe`、`gvim.exe` | 同时进入「已知编辑器」白名单（窗口标题可以从这里提取文件名） |
| 浏览器 | `chrome.exe`、`msedge.exe`、`firefox.exe` | **只取应用名，标题一律丢弃** |
| 音乐 | `cloudmusic.exe`、`QQMusic.exe`、`Spotify.exe` | |
| 游戏 | `GenshinImpact.exe`、`Yuanshen.exe` | 游戏名可以显示，它能给角色反应提供上下文 |
| 其他 | — | 只记录，不说话 |

映射表要做成**数据**（一张表），不要散在 `if` 分支里——以后要加分类只改这张表。

**八千代的人设已经定稿**：角色出自《超辉夜姬！》，依据与说话规则见
[八千代角色设定](../../八千代角色设定.md)。文案表在 `app/src-tauri/src/proactive.rs` 顶部，
系统提示词在 `app/src-tauri/src/ai/persona.rs`——**两处一起改**，别让对话里的她和气泡里的她像两个人。

### 8.6 聊天用的系统提示词

`ai/agent.rs` 组装消息时需要一个系统提示词。第一版写最小占位：

**已按角色设定稿写好**，在 `ai/persona.rs`：身份、对用户的称呼（「神明大人」）、以及说话的边界
（短、不带系统播报腔、不催促、不卖萌）。系统提示词里另外要拼上：当前工作区条目清单、当前模式、
以及（若开启）情境摘要。
